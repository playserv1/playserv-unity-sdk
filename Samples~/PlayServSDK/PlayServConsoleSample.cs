    ...
    public sealed class PlayServConsoleSample : MonoBehaviour
    {
        ...
        private async Task BindAsync(string playerId, bool useTransport)
        {
            ...
            _player = useTransport
                ? await PlayServData.SelectEntity<Player, SamplePlayerDto>(playerId, map, DataSubscriptionMode.Transport)
                : await PlayServData.SelectEntity<Player, SamplePlayerDto>(playerId, map);
        ...
        private void SubscribeSampleEvent()
        {
            ...
            _sampleEventSubscription = PlayServEvents.Subscribe<SampleChatEvent>(OnSampleEventReceived);
        ...
        {
            PlayServEvents.Publish(new SampleChatEvent
            {
        ...
        {
            PlayServEvents.PublishForGroup(groupName, new SampleChatEvent
            {
                SenderId = PlayServ.Settings.UserId,
                Text = text,
                SentAtUnixMs = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()
        ...
        {
            PlayServEvents.PublishForUser(userId, new SampleChatEvent
            {
                SenderId = PlayServ.Settings.UserId,
                Text = text,
                SentAtUnixMs = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()
