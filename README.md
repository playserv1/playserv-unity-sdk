...
This checkout prepares **0.6.8**. The examples below target that version, but do
...
4. Open `Window` -> `Package Manager`, choose `+` -> `Add package by name`
   ...
   and version `0.6.8`, then install. You can also find it under `My Registries`.
...
in `Packages/manifest.json` with the version string `"0.6.8"`; do not add a
...
The small buttons beside the SDK version check for updates, install an available update,
and open Package Manager. Hover an icon for its action, or the single-line status for
the complete message. Checking and installing disable the corresponding actions;
installing still requires confirmation.
...
This checkout targets SDK `0.6.8`. In Unity, open `Window` ->
`Package Manager`, choose `Add package from git URL`, and paste this pinned core
package URL:

```text
git@github.com:playserv1/playserv-unity-sdk.git#0.6.8
```

The Git URLs below require the `0.6.8` distribution tag to be published first;
...
`https://github.com/playserv1/playserv-unity-sdk.git#0.6.8` instead. The same
...
{
  "dependencies": {
    "com.playserv.sdk": "git@github.com:playserv1/playserv-unity-sdk.git#0.6.8"
  }
}
```
...
```json
{
  "dependencies": {
    "com.playserv.sdk": "git@github.com:playserv1/playserv-unity-sdk.git#0.6.8",
    "com.playserv.apple-signin": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.apple-signin#0.6.8",
    "com.playserv.google-signin": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.google-signin#0.6.8",
    "com.playserv.facebook-login": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.facebook-login#0.6.8",
    "com.playserv.epic-auth": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.epic-auth#0.6.8",
    "com.playserv.steam-auth": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.steam-auth#0.6.8",
    "com.playserv.webrtc": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.webrtc#0.6.8",
    "com.playserv.analytics": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.analytics#0.6.8",
    "com.playserv.pulse": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.pulse#0.6.8",
    "com.playserv.debug-terminal": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.debug-terminal#0.6.8",
    "com.playserv.transports-native": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.transports-native#0.6.8",
    "com.playserv.game-server": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.game-server#0.6.8",
    "com.playserv.schema-tool": "git@github.com:playserv1/playserv-unity-sdk.git?path=/CompanionPackages~/com.playserv.schema-tool#0.6.8"
  }
}
```
...
your project's registry with a one-hour credential. After connecting, select an
existing server or use **Create game server** to register a C# multi-room server
and select it for building. Server Token below Client Token is visible and saves
locally as you edit; empty the field to clear it. The image path uses a local
...
Optional [`PlayServRecordRef<T>`](Documentation~/record-references.md) values add lazy,
scoped relation loading and expanded previews while writing only IDs. Load children
explicitly before editing; saving a parent never saves its children.
